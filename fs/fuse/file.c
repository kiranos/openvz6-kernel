			 ...
			 loff_t offset, loff_t length)
{
	...
	if (mode & FALLOC_FL_PUNCH_HOLE) {
		pgoff_t start = (offset + PAGE_CACHE_SIZE - 1) >>
						PAGE_CACHE_SHIFT;
		pgoff_t end = (offset + length) >> PAGE_CACHE_SHIFT;
		unsigned int partial_start = offset & (PAGE_CACHE_SIZE - 1);
		unsigned int partial_end = (offset + length) &
						(PAGE_CACHE_SIZE - 1);
		struct page *page;

		/*
		 * truncate_pagecache_range() only handles page aligned ranges,
		 * so we have to zero partial segments by ourselves.
		 */
		if (end > start)
		...
		if (partial_start &&
		    (page = find_lock_page(inode->i_mapping, start - 1))) {
		...
		if (partial_end &&
		    (page = find_lock_page(inode->i_mapping, end))) {
			wait_on_page_writeback(page);
